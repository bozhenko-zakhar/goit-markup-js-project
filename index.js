...
`;function ye(){ge.innerHTML=fe,_e.innerHTML=be}ye();let X="682f9bbf8acbdf505592ac36";const d=document.querySelector("[data-modal]"),G=document.querySelector("[data-modal-close]"),C=document.querySelector(".adopt-modal-form"),ve=document.querySelector("[data-modal-close]");ve.innerHTML=`
<svg class="adopt-modal-svg" width="24" height="24">
	<use href="${u}#icon-close"></use>
</svg>
`;function he(t){t&&(X=t),d==null||d.classList.remove("is-hidden"),document.body.classList.add("modal-open"),E()}function q(){d==null||d.classList.add("is-hidden"),document.body.classList.remove("modal-open")}G&&G.addEventListener("click",q);d&&d.addEventListener("click",t=>{t.target===d&&q()});document.addEventListener("keydown",t=>{t.key==="Escape"&&!(d!=null&&d.classList.contains("is-hidden"))&&q()});C&&(C.setAttribute("novalidate","true"),C.addEventListener("submit",async t=>{var l;t.preventDefault();const{name:e,phone:s,comment:i}=t.target.elements,a=s.value.replace(/\D/g,"");if(a.length!==12){h.fire({icon:"error",title:"Помилка",text:"Телефон має містити 12 цифр"});return}if(!e.value.trim()){h.fire({icon:"warning",title:"Помилка заповнення",text:"Будь ласка, введіть ваше ім’я"});return}const o={name:e.value.trim(),phone:a,animalId:X,comment:i.value.trim()||"Запит на знайомство"};try{h.showLoading();const p=await k.post("https://paw-hut.b.goit.study/api/orders",o);console.log("orderData :>> ",p.data),await h.fire({icon:"success",title:"Дякуємо за Вашу заявку!",text:"Запис на знайомство прийнято."}),t.target.reset(),q()}catch(p){console.error("SERVER ERROR:",(l=p.response)==null?void 0:l.data),h.fire({icon:"error",title:"Помилка сервера",text:"Спробуйте пізніше"})}}));const m={backdrop:document.querySelector(".animal-detail-backdrop")};document.addEventListener("click",t=>{const e=t.target.closest(".product-card__btn--learnMore");if(!e)return;const s=e.dataset.modalId;we(s)});function we(t){const e=I.animals.get(t);e&&(m.backdrop.innerHTML=ke(e),m.backdrop.classList.add("is-open"),document.body.classList.add("not-scroll"),m.dataBtn=document.querySelector("[data-btn]"),m.dataBtn.addEventListener("click",()=>{he(t),E()}))}function ke({image:t,species:e,name:s,age:i,gender:a,description:o,healthStatus:l,behavior:p}){return`
...
`}function E(){m.backdrop.classList.remove("is-open"),document.body.classList.remove("not-scroll")}m.backdrop.addEventListener("click",t=>{(t.target===m.backdrop||t.target.closest(".animal-detail-close-btn"))&&E()});document.addEventListener("keydown",t=>{t.key==="Escape"&&E()});
