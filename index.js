...
`).join("");c.categoryList.innerHTML=t},b=e=>{const t=e.map(({_id:a,image:o,species:s,name:n,categories:r,age:u,gender:F,behavior:U})=>{const z=`
    ...
    `}return`<div class="rating">${o}</div>`}async function de(){try{const t=await(await fetch("https://paw-hut.b.goit.study/api/feedbacks")).json(),a=Array.isArray(t)?t.slice(0,6):t.feedbacks.slice(0,6);a.length&&(ue(a),me())}catch(e){console.error("Помилка завантаження відгуків:",e)}}function ue(e){ce.innerHTML=e.map(t=>`
      ...
      `).join("")}function me(){new R(".testimonials-swiper",{modules:[x,N],slidesPerView:1,spaceBetween:32,loop:!1,observer:!0,observeParents:!0,navigation:{nextEl:".testimonials-next",prevEl:".testimonials-prev"},pagination:{el:".testimonials-pagination",clickable:!0},breakpoints:{704:{slidesPerView:2}}})}de();const p={backdrop:document.querySelector(".animal-detail-backdrop")};document.addEventListener("click",e=>{const t=e.target.closest(".product-card__btn--learnMore");if(!t)return;console.log(1);const a=t.dataset.modalId;pe(a)});function pe(e){const t=h.animals.get(e);t&&(p.backdrop.innerHTML=ge(t),p.backdrop.classList.add("is-open"))}function ge({image:e,species:t,name:a,age:o,gender:s,description:n,healthStatus:r,behavior:u}){return`
...
`}p.backdrop.addEventListener("click",e=>{(e.target===p.backdrop||e.target.closest(".animal-detail-close-btn"))&&p.backdrop.classList.remove("is-open")});document.addEventListener("keydown",e=>{e.key==="Escape"&&p.backdrop.classList.remove("is-open")});let fe="682f9bbf8acbdf505592ac36";const l=document.querySelector("[data-modal]"),I=document.querySelector("[data-modal-close]"),E=document.querySelector(".adopt-modal-form");function k(){l==null||l.classList.add("is-hidden"),document.body.classList.remove("modal-open")}I&&I.addEventListener("click",k);l&&l.addEventListener("click",e=>{e.target===l&&k()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!(l!=null&&l.classList.contains("is-hidden"))&&k()});E&&(E.setAttribute("novalidate","true"),E.addEventListener("submit",async e=>{var r;e.preventDefault();const{name:t,phone:a,comment:o}=e.target.elements,s=a.value.replace(/\D/g,"");if(s.length!==12){g.fire({icon:"error",title:"Помилка",text:"Телефон має містити 12 цифр"});return}if(!t.value.trim()){g.fire({icon:"warning",title:"Помилка заповнення",text:"Будь ласка, введіть ваше ім’я"});return}const n={name:t.value.trim(),phone:s,modelId:fe,color:"#1212ca",comment:o.value.trim()||"Запит на знайомство"};try{g.showLoading();const u=await _.post("https://furniture-store.b.goit.study/api/orders",n);console.log("orderData :>> ",u.data),await g.fire({icon:"success",title:"Дякуємо за Вашу заявку!",text:"Запис на знайомство прийнято."}),e.target.reset(),k()}catch(u){console.error("SERVER ERROR:",(r=u.response)==null?void 0:r.data),g.fire({icon:"error",title:"Помилка сервера",text:"Спробуйте пізніше"})}}));
